           ...
           if (currentTool == Tool.PICKER)
            {
                ...
                {
                    robot = new Robot();
                    SelecionMateria.paint.paintTools.toolOptionPanel.setColor( robot.getPixelColor(pointer.getLocation().x, pointer.getLocation().y) );

                    if (mousePressed)
                    {
                        setBrushColor(SelecionMateria.paint.paintTools.toolOptionPanel.getColor());
                        SelecionMateria.paint.colorPicker.currentClrPanel.setBackground(SelecionMateria.paint.paintTools.toolOptionPanel.getColor());
                        tool.setColor(SelecionMateria.paint.paintTools.toolOptionPanel.getColor());
                        repaint();
                    }
                    robot = null;
                }
    ...
    public void setPaintTool(Tool tool)
    {
        ...
        switch (tool)
        {
            case PENCIL:
            {
                ...
                SelecionMateria.paint.paintTools.setToolOptionPanel(new PencilToolPanel(Tool.PENCIL, this.tool.getStroke()));
                break;
            }
            case BRUSH:
            {
                this.tool = new BrushTool(brushColor, this.tool.getStroke(), StrokeStyle.LINE);
                SelecionMateria.paint.paintTools.setToolOptionPanel(new BrushToolPanel(Tool.BRUSH, this.tool.getStroke()));
                break;
            }
            case AIRBRUSH:
            {
                this.tool = new AirBrushTool(brushColor, this.tool.getStroke());
                SelecionMateria.paint.paintTools.setToolOptionPanel(new AirBrushToolPanel(Tool.AIRBRUSH,this.tool.getStroke()));
                break;
            }
            case ERASER:
            {
                this.tool = new EraserTool(getBackground(), this.tool.getStroke(), StrokeStyle.SQUARE);
                SelecionMateria.paint.paintTools.setToolOptionPanel(new EraserToolPanel(Tool.ERASER,this.tool.getStroke()));
                break;
            }
            case RECTANGLE:
            {
                this.tool = new RectangleTool(brushColor, this.tool.getStroke());
                SelecionMateria.paint.paintTools.setToolOptionPanel(new RectShapeToolPanel(Tool.RECTANGLE,this.tool.getStroke()));
                break;
            }
            case ROUND_RECT:
            {
                this.tool = new RoundRectTool(brushColor, this.tool.getStroke());
                SelecionMateria.paint.paintTools.setToolOptionPanel(new RectShapeToolPanel(Tool.ROUND_RECT,this.tool.getStroke()));
                break;
            }
            case OVAL:
            {
                this.tool = new OvalTool(brushColor, this.tool.getStroke());
                SelecionMateria.paint.paintTools.setToolOptionPanel(new RectShapeToolPanel(Tool.OVAL,this.tool.getStroke()));
                break;
            }
            case LINE:
            {
                this.tool = new LineTool(brushColor, this.tool.getStroke(), StrokeStyle.LINE);
                SelecionMateria.paint.paintTools.setToolOptionPanel(new DragShapeToolPanel(Tool.LINE,this.tool.getStroke()));
                break;
            }
            case FILLER:
            {
            ...
            {
                this.tool = new FillerTool(brushColor, this.tool.getStroke());
                SelecionMateria.paint.paintTools.setToolOptionPanel(new ColorPickerToolPanel(Tool.PICKER, brushColor));
                break;
            }
        }
    }
